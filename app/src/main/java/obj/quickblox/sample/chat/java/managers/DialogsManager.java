package obj.quickblox.sample.chat.java.managers;
...
public class DialogsManager {
    ...
    private QBChatMessage buildMessageCreatedGroupDialog(QBChatDialog dialog) {
        ...
        qbChatMessage.setBody(getString(R.string.new_chat_created)+" "+getCurrentUserName());
    ...
    private QBChatMessage buildMessageAddedUsers(QBChatDialog dialog, String userIds, String usersNames) {
        ...
        qbChatMessage.setBody(getString(R.string.occupant_added)+" "+getCurrentUserName()+" "+usersNames);
        qbChatMessage.setSaveToHistory(true);
        qbChatMessage.setMarkable(true);
        return qbChatMessage;
    }
    ...
    private QBChatMessage buildMessageLeftUser(QBChatDialog dialog) {
        ...
        qbChatMessage.setBody(getString(R.string.occupant_left)+" "+getCurrentUserName());
        qbChatMessage.setSaveToHistory(true);
        qbChatMessage.setMarkable(true);
        return qbChatMessage;
    }
    private QBChatMessage settheame(QBChatDialog dialog,String s) {
        QBChatMessage qbChatMessage = new QBChatMessage();
        qbChatMessage.setDialogId(dialog.getDialogId());
        qbChatMessage.setProperty(PROPERTY_NOTIFICATION_TYPE, OCCUPANT_LEFT);
        qbChatMessage.setBody(s);
        qbChatMessage.setSaveToHistory(true);
        qbChatMessage.setMarkable(true);
        return qbChatMessage;
    }
    ...
    //Theam Set
    public void sendMessageTheamSet(QBChatDialog dialog,String s) {
        QBChatMessage messageTheamSet = settheame(dialog,s);
        try {
            Log.d(TAG, s);
            dialog.sendMessage(messageTheamSet);
        } catch (SmackException.NotConnectedException ignored) {
    ...
    public void sendSystemMessageTHEAM(final QBSystemMessagesManager systemMessagesManager, final QBChatDialog dialog,String s) {
        QBChatMessage messageLeftUser = settheame(dialog,s);
        //prepareSystemMessage(systemMessagesManager, messageLeftUser, dialog.getOccupants());
    }
